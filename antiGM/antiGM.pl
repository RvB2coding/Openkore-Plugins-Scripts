...
use Globals qw($char $bus $field %config @servers %statusHandle $taskManager);
...
my $gm_pattern = '^(\[GM\]|\[GE\]|\[LU\]|\[CM\]|\[ADM\]|\[Admin\])';
my @gm_ids = (2000001, 2000002, 2000003);
my $plugin_folder = $Plugins::current_plugin_folder;
my $plugin_audio = "antigm.mp3";
...
sub GMreaction {
	...
	$taskManager->add(
			new Task::Wait('name' => 'pause', 'priority' => Task::LOW_PRIORITY, 'seconds' => 60, 'inGame' => 1)
			);
