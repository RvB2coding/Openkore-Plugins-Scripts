...
use base qw(Network::Send::Zero);
use Data::Dumper;

Plugins::register('ZeroAuth', 'kRO Zero SSO Authenticator', \&unload);

use constant {
	WEBSITE_URL => 'http://roz.gnjoy.com/',
...
sub sendMasterLogin {
	...
	$headers = HTTP::Headers->new(
		...
		'Referer' => 'http//roz.gnjoy.com/index.asp',
	...
	$req = POST $url, [
		...
		rtnurl => 'http%3A%2F%2Froz%2Egnjoy%2Ecom%2Findex%2Easp'
	];
	$response = $ua->request($req);
	$cookie_jar->extract_cookies($response);	
	...
	$ua->default_headers($headers);
	$url = WEBSITE_URL.'GameExecute/Execute.asp?gamecode=0036';
	$req = GET $url;		
	$response = $ua->request($req);
	$cookie_jar->extract_cookies( $response );
	$ua->cookie_jar($cookie_jar);
	...
	if($cookie_jar->as_string =~/t1\=(\w+)\&/) {		
		...
		if($response->decoded_content =~/\w+\|(\w+)\|\w+\|\w+\|\w+/) {
			if($1 ne $current_token) { 
				...
				my $len =  length($1) + 92;				
				$messageSender->sendTokenToServer($config{username}, $config{password}, $master->{master_version}, $master->{version}, $1, $len, $master->{OTT_ip}, $master->{OTT_port});
			}
		}
...
*Network::Send::Zero::sendMasterLogin = *ZeroAuth::sendMasterLogin;
